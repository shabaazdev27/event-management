...
export default function LiveWaitTimes() {
  ...
  return (
    <section className="bg-neutral-900/50 border border-white/5 rounded-2xl p-6 backdrop-blur-sm w-full shrink-0 relative z-0 isolate">
      <div className="flex items-center gap-2 mb-4 text-indigo-400">
        <Clock className="w-5 h-5" />
        <h2 className="font-semibold">Live Wait Times</h2>
        {loading && (
          <span className="text-xs ml-auto text-neutral-500 animate-pulse">
            Connecting…
          </span>
        )}
      </div>
      ...
      <div className="space-y-4">
        {queues.length === 0 && !loading && (
          <div className="text-sm text-neutral-500 italic p-2 text-center border border-dashed border-white/10 rounded-xl">
...
function QueueItem({
  ...
  return (
    <div className="flex justify-between items-center gap-3 p-3 rounded-xl bg-neutral-800/40 border border-white/5 min-w-0">
      ...
      <span
        className={`text-xs font-semibold px-2 py-1 rounded-md border flex-shrink-0 ${colors[status] || colors.moderate}`}
      >
        {time}
      </span>
    </div>
