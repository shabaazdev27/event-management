  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        {events.length > 0 && (
          <div className="flex items-center gap-2 bg-neutral-950/50 px-3 py-1.5 rounded-lg border border-white/10 w-full sm:w-auto">
            <Calendar className="w-4 h-4 text-neutral-400 shrink-0" aria-hidden />
          ...
          {schedule.map((item, idx) => (
            <div key={idx} className="relative flex gap-4">
              {idx !== schedule.length - 1 && (
                <div
                  className={`absolute top-8 left-3 w-0.5 h-full -ml-px ${item.done ? "bg-indigo-500/50" : "bg-white/5"}`}
                  aria-hidden
                />
              )}
              ...
              >
                <div className="flex justify-between items-start mb-1 flex-wrap gap-1">
                  <h3
                    className={`font-medium pr-2 max-w-full wrap-break-word ${item.active ? "text-indigo-300" : "text-white"}`}
                  >
                    {item.title}
                  </h3>
                  <span className="text-[10px] sm:text-xs font-mono text-neutral-400 bg-neutral-800 px-2 py-0.5 rounded shrink-0">
                    {item.time}
                  </span>
                </div>
      ...
      <div className="mt-4 p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-sm flex gap-3 items-center">
        <Ticket className="w-5 h-5 text-indigo-400 shrink-0" aria-hidden />
